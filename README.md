...
├── memoria.js          → Memoria permanente + hechos aprendidos; construye las secciones del system prompt (extraído de app.js en v203)
├── viernes.js          → VIERNES: segundo cerebro analítico. Lee el histórico y predice (agotamiento, ritmo por tienda, ciclos de resurtido). Local puro: sin API, sin red, solo lectura
...
├── forja.js            → FORJA: hologramas 3D sobre la cámara — diseño por IA (Groq), anclaje por giroscopio, manipulación por pellizco
...
├── sw.js               → Service Worker v212 (PWA / modo offline / cache network-first)
├── manifest.json       → Manifest PWA (instalable en móvil/escritorio)
├── vendor/             → Librerías autoalojadas (marked 9.1.6, DOMPurify 3.0.6): el chat sobrevive a la caída de sus CDN
...
| Groq API — GPT-OSS 120B (cascada llama-3.3-70b → llama-3.1-8b) | Motor de IA principal del chat gratis en Groq; la cascada salta modelos retirados (404 o 400 model_decommissioned). Nota: Kimi K2 fue dado de baja por Groq en 2026 |
...
| PWA + Service Worker v212 | Instalable, network-first para shell, cache offline |
